...
export const ImageEditor: React.FC<ImageEditorProps> = ({ initialImageUrl, onClearInitialImage, onImageTransfer }) => {
    ...
    const handleApiError = (e: any) => {
        ...
        setError(e.message || 'Échec de l\'édition de l\'image.');
    };
    ...
    const handleClearAllImages = () => {
        sourceImageUrls.forEach(url => {
            if(url) URL.revokeObjectURL(url);
        });
        setSourceImageFiles(Array(3).fill(null));
        setSourceImageUrls(Array(3).fill(null));
        setEditedImage(null);
        setError(null);
    };
    ...
    const handleGenerate = async () => {
        if (!prompt) {
            setError('Veuillez entrer une description des modifications.');
            return;
        }
        const activeSourceFiles = sourceImageFiles.filter((f): f is File => f !== null);
        if (activeSourceFiles.length === 0) {
            setError('Veuillez fournir au moins une image à modifier.');
            return;
        }
        ...
        try {
            ...
            if (firstPart && 'inlineData' in firstPart && firstPart.inlineData) {
                ...
                setEditedImage(`data:${generatedMimeType};base64,${generatedBase64}`);
            } else {
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 max-w-6xl">
            <h2 className="text-3xl font-bold text-center mb-6 text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-purple-400">Éditeur d'Image</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                 <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h3 className="text-xl font-semibold">Images de Base</h3>
                        <button onClick={handleClearAllImages} className="text-sm text-cyan-400 hover:text-cyan-300">
                            Changer les images
                        </button>
                    </div>
                 ...
                 <div className="space-y-4">
                     <h3 className="text-xl font-semibold text-center">Image Modifiée</h3>
                    <div className="relative group aspect-square bg-gray-800 rounded-xl flex items-center justify-center">
                       ...
                       {editedImage && (
                           <>
                                <img src={editedImage} alt="Edited" className="rounded-xl w-full h-full object-contain" />
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-4">
                                    ...
                                    <div className="flex flex-wrap items-center justify-center gap-3">
                                        ...
                                        >
                                         ...
                                         <button 
                                            onClick={() => onImageTransfer(editedImage, 'emotionGenerator')}
                                        ...
                                        <button 
                                            onClick={() => onImageTransfer(editedImage, 'cameraAngles')}
                                        ...
                                        <button 
                                            onClick={() => onImageTransfer(editedImage, 'imageEditing')}
                                        ...
                                        <button 
                                            onClick={() => onImageTransfer(editedImage, 'videoGeneration')}
                       ...
                       {!isLoading && !editedImage && !error && <p className="text-gray-500">Votre image modifiée apparaîtra ici.</p>}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-800/50 p-6 rounded-2xl border border-gray-700 space-y-4">
                <textarea
                    ...
                    placeholder="Décrivez les modifications que vous souhaitez apporter (ex: 'change the color of the car to red', 'add a cat sitting on the roof')..."
                ...
                <button
                    ...
                    disabled={isLoading || !prompt || !hasSourceImages}
                    className="w-full flex items-center justify-center px-6 py-3 font-semibold text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors disabled:bg-purple-800 disabled:cursor-not-allowed"
                >
                    {isLoading ? <><LoadingSpinner /> <span className="ml-2">Édition en cours...</span></> : 'Appliquer les Modifications'}
                </button>
            </div>
            <FullScreenModal
