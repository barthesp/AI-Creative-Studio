...
import { EMOTIONS, IMAGE_STYLES, CREATIVE_MODE_PROMPT, FIDELITY_PROMPT } from '../constants';
import { Emotion, AppMode, GeneratedImage } from '../types';
...
export const EmotionGenerator: React.FC<EmotionGeneratorProps> = ({ onImageTransfer, initialImageUrl, onClearInitialImage }) => {
    ...
    const handleClear = () => {
        ...
        setGeneratedImages(EMOTIONS.map(emotion => ({
            id: emotion.id,
            name: emotion.name,
            url: null,
            isLoading: false,
    ...
    const handleGenerate = useCallback(async (emotion: Emotion) => {
        if (!originalImageFile) return;

        setGeneratedImages(prev => prev.map(img =>
            img.id === emotion.id ? { ...img, isLoading: true, error: null, url: null } : img
        ));
        ...
        try {
            ...
            const fullPrompt = `${emotion.prompt}. ${stylePrompt}. ${modePrompt}`;
            ...
            if (firstPart && 'inlineData' in firstPart && firstPart.inlineData) {
                ...
                setGeneratedImages(prev => prev.map(img =>
                    img.id === emotion.id ? { ...img, url: imageUrl, isLoading: false } : img
                ));
            } else {
            ...
            setGeneratedImages(prev => prev.map(img =>
                img.id === emotion.id ? { ...img, isLoading: false, error: errorMessage } : img
            ));
        }
    ...
    const handleGenerateAll = async () => {
        ...
        for (const emotion of EMOTIONS) {
            const existing = generatedImages.find(img => img.id === emotion.id);
            if (!existing || (!existing.url && !existing.isLoading && !existing.error)) {
                 await handleGenerate(emotion);
            }
        }
        setIsGeneratingAll(false);
    };
